...
@router.get("/by-skus")
async def get_consequents_by_skus_endpoint( skus: str = Query(..., description="Lista de SKUs separados por coma"),
    db_connection: pyodbc.Connection = Depends(get_mssql_connection)) -> Any:
...
async def get_consequents_by_skus_endpoint( skus: str = Query(..., description="Lista de SKUs basados en codigo supabase"),
    db_connection: pyodbc.Connection = Depends(get_mssql_connection)) -> Any:
    # Convertir el string de SKUs separados por coma a lista
    skus_list = [sku.strip() for sku in skus.split(",") if sku.strip()]
