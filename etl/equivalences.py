...
class ProductInfo:
    ...
    source: str
    source_code: (
        str  # The code used by this source (SKU, codigo_alt, codigo_mongo, etc.)
    )
...
class EquivalenceMap:
    ...
    def _normalize_key(self, nombre: str, categoria: str) -> tuple:
        ...
        return ((nombre or "").strip().lower(), (categoria or "").strip().lower())

    def _get_or_create_equivalence(
    ...
    def add_mssql_products(self, productos: List[Any], debug: bool = False):
        ...
        for i, p in enumerate(productos):
            ...
            if debug and i < 3:
                print(
                    f"        MSSQL sample {i}: SKU={sku}, nombre={nombre[:30] if nombre else 'EMPTY'}, cat={categoria}"
                )
    ...
    def add_mysql_products(self, productos: List[Any], debug: bool = False):
        ...
        for i, p in enumerate(productos):
            ...
            if debug and i < 3:
                print(
                    f"        MySQL sample {i}: codigo_alt={codigo_alt}, nombre={nombre[:30] if nombre else 'EMPTY'}, cat={categoria}"
                )
    ...
    def add_supabase_products(
        self, productos: List[Dict[str, Any]], debug: bool = False
    ):
        """
        ...
        for i, p in enumerate(productos):
            ...
            if debug and i < 3:
                print(
                    f"        Supabase sample {i}: sku={sku}, nombre={nombre[:30] if nombre else 'EMPTY'}, cat={categoria}"
                )
    ...
    def add_mongo_products(self, productos: List[Dict[str, Any]], debug: bool = False):
        ...
        for i, p in enumerate(productos):
            ...
            if debug and i < 3:
                print(
                    f"        MongoDB sample {i}: codigo_mongo={codigo_mongo}, nombre={nombre[:30] if nombre else 'EMPTY'}, cat={categoria}"
                )
    ...
    def add_neo4j_products(self, productos: List[Dict[str, Any]], debug: bool = False):
        ...
        for i, p in enumerate(productos):
            ...
            if debug and i < 3:
                print(
                    f"        Neo4j sample {i}: sku={sku}, nombre={nombre[:30] if nombre else 'EMPTY'}, cat={categoria}"
                )
    ...
    def get_equivalence(
        self, nombre: str, categoria: str
    ) -> Optional[ProductEquivalence]:
        """Get the full equivalence information for a product."""
        key = self._normalize_key(nombre, categoria)
        return self._by_name_cat.get(key)

    def get_equivalence_by_source(
        self, source: str, source_code: str
    ) -> Optional[ProductEquivalence]:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for eq in self._by_name_cat.values():
            ...
            if "mssql" in eq.sources and eq.sources["mssql"].sku == eq.sku_oficial:
                ...
                "supabase" in eq.sources
                and eq.sources["supabase"].sku == eq.sku_oficial
            ):
                sku_from["supabase"] += 1
...
def build_equivalence_map(
    ...
    if productos_mssql:
        ...
        if debug:
            print(
                f"      MSSQL: added {len(productos_mssql)} products, unique now: {len(eq_map)} (+{len(eq_map) - before})"
            )
        before = len(eq_map)
    ...
    if productos_mysql:
        ...
        if debug:
            print(
                f"      MySQL: added {len(productos_mysql)} products, unique now: {len(eq_map)} (+{len(eq_map) - before})"
            )
        before = len(eq_map)
    ...
    if productos_supabase:
        ...
        if debug:
            print(
                f"      Supabase: added {len(productos_supabase)} products, unique now: {len(eq_map)} (+{len(eq_map) - before})"
            )
        before = len(eq_map)
    ...
    if productos_mongo:
        ...
        if debug:
            print(
                f"      MongoDB: added {len(productos_mongo)} products, unique now: {len(eq_map)} (+{len(eq_map) - before})"
            )
        before = len(eq_map)
    ...
    if productos_neo4j:
        ...
        if debug:
            print(
                f"      Neo4j: added {len(productos_neo4j)} products, unique now: {len(eq_map)} (+{len(eq_map) - before})"
            )
