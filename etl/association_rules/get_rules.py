...
def transformar_a_one_hot(df):
    ...
    print(
        f"    Transacciones después de filtrar: {df_exploded['transaction_id'].nunique()}"
    )
...
def generar_reglas_asociacion():
    try:
        ...
        if frequent_itemsets.empty:
            print(
                "❌ No se encontraron itemsets frecuentes. Intenta reducir MIN_SUPPORT."
            )
            return None
        ...
        print(
            f"    Generando reglas con min_confidence={MIN_CONFIDENCE}, min_lift={MIN_LIFT}..."
        )
